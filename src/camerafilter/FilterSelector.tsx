...
export function FilterSelector({ onSelect, onCapturePress, onCaptureLongPress, onCapturePressOut, isRecording, recordingProgressAnim }: Props): React.JSX.Element {
  ...
  const handlePressItem = (id: FilterId, index: number) => {
    if (selected === id) {
      onCapturePress?.();
    } else {
      setSelected(id);
  ...
  const handleLongPressItem = (id: FilterId) => {
    if (selected === id) {
      onCaptureLongPress?.();
    }
  };

  const handlePressOutItem = (id: FilterId) => {
    if (selected === id) {
      onCapturePressOut?.();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.scrollWrapper}>
        <View style={styles.staticRingContainer} pointerEvents="none">
          {isRecording && recordingProgressAnim ? (
            <View style={{ width: 84, height: 84, marginTop: -9 }}>
      ...
      >
        <Pressable
          onPress={() => handlePressItem('none', 0)}
          onLongPress={() => handleLongPressItem('none')}
          onPressOut={() => handlePressOutItem('none')}
          delayLongPress={200}
          ...
          return (
            <Pressable
              ...
              onPress={() => {
                if (isActive) {
                  onCapturePress?.();
                } else if (option.id === 'glasses') {
                  handlePressItem('glasses_classic', index + 1);
                } else if (option.id === 'lipstick') {
                  handlePressItem('lipstick_red', index + 1);
                } else {
                  handlePressItem(option.id, index + 1);
                }
              }}
              onLongPress={() => {
                if (isActive) onCaptureLongPress?.();
              }}
              onPressOut={() => {
                if (isActive) onCapturePressOut?.();
              }}
              delayLongPress={200}
              style={styles.itemContainer}
            >
